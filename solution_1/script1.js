...
function generatePassword (options) {
    ...
    for (var i=0; i<options.passwordLength; i++){
        if (requiredCharacters[i]){
            ...
            var possibleCharactersString = possibleCharacters[Math.floor( Math.random()*possibleCharacters.length)];
            password += possibleCharactersString.charAt(Math.floor( Math.random()*possibleCharactersString.length));
        }    
    }
    return password;
}
